...
class PlanningWizardView(LoginRequiredMixin, SessionWizardView):
    ...
    # Create and save TripBudget
        ...
        messages.success(
            self.request, "Your trip has been successfully created!"
            )
        ...
        if formset.is_valid():
            ...
            for instance in instances:
                ...
                if not instance.day_number:
                    max_day = trip.itinerary_days.aggregate(
                        Max("day_number")
                    ).get("day_number__max", 0) or 0
...
class EditTripWizardView(LoginRequiredMixin, SessionWizardView):
    ...
    def done(self, form_list, **kwargs):
        ...
        TripDetails.objects.update_or_create(
            trip=trip,
            defaults={
                'accommodation_type': trip_details_form_data[
                    'accommodation_type'
                    ],
                'transport_type': trip_details_form_data[
                    'transport_type'
                    ]
            }
        )
...
def suggest_activity(request, day_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Activity suggestion added successfully!"
                )
